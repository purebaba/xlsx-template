...
module.exports = (function () {
     ...
     */
    var Workbook = function (data, options = {}) {
        var self = this;

        self.archive = null;
        self.sharedStrings = [];
        self.sharedStringsLookup = {};
        self.imageOptions = options.image || {}
    ...
    Workbook.prototype.deleteSheet = function (sheetName) {
        ...
        var id = self.sheets.findIndex(function (el) {
            return el.id === sheet.id
        });
                ...
                if (cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(function (placeholder) {
                        ...
                        if (placeholder.full && placeholder.type === "table" && substitution instanceof Array) {
                            ...
                            // if input type is string, insert cell with string ,not insert image
                            ...
                            string = self.substituteImage(cell, string, placeholder, substitution, drawing, sheet.root);
    ...
    Workbook.prototype.substituteImage = function (cell, string, placeholder, substitution, drawing, sheet) {
        var self = this;
        ...
        let {padding} = self.imageOptions
        if (padding) {
            padding = self.pixelsToEMUs(padding)
        } else {
            padding = 0
        }
        ...
        fromColOff.text = (0 + padding).toString()
        ...
        fromRowOff.text = (0 + padding).toString()
        ...
        toColOff.text = (0 - padding).toString()
        toRowOff.text = (0 - padding).toString()

        // fill image in mergeCell
        let mergeCell = sheet.findall("mergeCells/mergeCell").find(mergeCell => {
            return cell.attrib.r === self.splitRange(mergeCell.attrib.ref).start
        })

        if (mergeCell) {
            let mergeRange = self.splitRange(mergeCell.attrib.ref)
            let mergeEnd = self.splitRef(mergeRange.end)
            toCol.text = (Number.parseInt(self.charToNum(mergeEnd.col))).toString();
            toRow.text = (Number.parseInt(mergeEnd.row)).toString();
        } else {
            toCol.text = (Number.parseInt(self.charToNum(self.splitRef(cell.attrib.r).col))).toString();
            toRow.text = (Number.parseInt(self.splitRef(cell.attrib.r).row)).toString();
        }
        ...
        var useLocalDpi = etree.SubElement(ext, 'a14:useLocalDpi', {
